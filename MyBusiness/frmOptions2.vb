...
#Region "Form event handlers"
    ...
    Private Sub ceOptions_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        LoadFolderOptions()
        LoadEmailOptions()
        SpellCheckUtil.EnableSpellChecking(New Control() {txtSpellTest})
...
#Region "Invoice"
    Private Sub LoadInvoiceOptions()
        For Each sFontName In [Enum].GetValues(GetType(RtfManager.RtfFontType)).Cast(Of RtfManager.RtfFontType).ToList()
            cbInvHdrFont.Items.Add(sFontName.ToString.Replace("_", " "))
            cbInvBodyFont.Items.Add(sFontName.ToString.Replace("_", " "))
            cbInvFtrFont.Items.Add(sFontName.ToString.Replace("_", " "))
        Next
        cbInvHdrFont.SelectedIndex = cbInvHdrFont.FindStringExact(My.Settings.InvHdrFont.FontFamily.GetName(0))
        cbInvBodyFont.SelectedIndex = cbInvBodyFont.FindStringExact(My.Settings.InvBodyFont.FontFamily.GetName(0))
        cbInvFtrFont.SelectedIndex = cbInvFtrFont.FindStringExact(My.Settings.InvFtrFont.FontFamily.GetName(0))
        nudInvHdrSize.Value = My.Settings.InvHdrFont.Size
        nudInvBodySize.Value = My.Settings.InvBodyFont.Size
        nudInvFtrSize.Value = My.Settings.InvFtrFont.Size



        txtInvHdrSample.Font = My.Settings.InvHdrFont
        txtInvBodySample.Font = My.Settings.InvBodyFont
        txtInvFtrSample.Font = My.Settings.InvFtrFont
    End Sub

    Private Sub SaveInvoiceOptions()
        My.Settings.InvHdrFont = txtInvHdrSample.Font
        My.Settings.InvBodyFont = txtInvBodySample.Font
...
#Region "Email"
    ...
    Private Sub LoadEmailOptions()
        If Not My.Settings.useSMTP Then
            Dim oOutlookAccounts As Outlook.Accounts = EmailUtil.getOutlookAccounts
            If oOutlookAccounts IsNot Nothing Then
                For Each account As Outlook.Account In oOutlookAccounts
                    cbOutlookAccount.Items.Add(account.SmtpAddress)
                Next
                cbOutlookAccount.SelectedItem = My.Settings.OutlookAccount
            End If
        End If
        ...
        chkSmtpCredentials.Checked = My.Settings.SMTPRequiresCredentials
        chkSmtpSsl.Checked = My.Settings.SMTPEnableSSL
    ...
    Private Sub SaveEmailOptions()
        ...
        End If
        If cbOutlookAccount.SelectedIndex > -1 Then
            My.Settings.OutlookAccount = cbOutlookAccount.SelectedItem
        End If
    End Sub

    Private Sub btnInvHdrFont_Click(ByVal sender As Object, ByVal e As EventArgs)
    ...
    Private Sub cbInvHdrFont_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cbInvHdrFont.SelectedIndexChanged, nudInvHdrSize.ValueChanged
        If Not isLoading Then txtInvHdrSample.Font = New Font(cbInvHdrFont.SelectedItem.ToString, nudInvHdrSize.Value)

    End Sub

    Private Sub cbInvBodyFont_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cbInvBodyFont.SelectedIndexChanged, nudInvBodySize.ValueChanged
        If Not isLoading Then txtInvBodySample.Font = New Font(cbInvBodyFont.SelectedItem.ToString, nudInvBodySize.Value)

    End Sub

    Private Sub cbInvFtrFont_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cbInvFtrFont.SelectedIndexChanged, nudInvFtrSize.ValueChanged
        If Not isLoading Then txtInvFtrSample.Font = New Font(cbInvFtrFont.SelectedItem.ToString, nudInvFtrSize.Value)
